...
(function($) {
    ...
    function GetCoordinateModel(mbMap, $inputCoords, $inputSrs, $resultCoords, $resultSrs, $button) {
        ...
        this.setResult = function() {
            ...
            if (data && data[0]) {
                ...
                var inputSrs = Mapbender.Model.getProj($inputSrs.val());
        ...
        this._handleClick = function(e){
            ...
            $resultCoords.val(parseFloat(lonlat.lon) + ' ' + parseFloat(lonlat.lat));
            this.setResult();
            //this.deactivate();
            return false;
        };
    ...
    function InputModel(mbMap, $inputCoords, $inputSrs, $resultCoords, $resultSrs, $button) {
        this.activated = false;
        this.feature = null;
        this.buffer = 1;
        this.getName = function(){
...
//            this.setResult();
        ...
        this.clickButton = function(e) {
            if(this.setResult()){
                var ordinates = $resultCoords.val().split(/[\s,]/);
            ...
            $('.buttonGroup [data-modelname]', this.element).each(function(idx, item){
                ...
                if($item.attr('data-modelname') === 'GetCoordinateModel'){
                    $item.data('model', new GetCoordinateModel(self.mbMap.map, $('input.inputCoordinate', self.element),
                        ...
                        $('input.mapSrs', self.element), $('.readFromMap', self.element)));
                    ...
                    $item.data('model', new InputModel(self.mbMap.map, $('input.inputCoordinate', self.element),
                        $('select.inputSrs', self.element), $('input.mapCoordinate', self.element),
                        $('input.mapSrs', self.element), $('.centerMap', self.element)));
                    $item.data('model').deactivate();
                }
            });
